...
// @version      11.23.1
// @description  Virag V11.23.1 Lite — exact visible-quantity prompt builder with Real Human Character Master.
// @author       Rohit
...
function syncplanfromui(){
  if(!S.s)return{products:S.pc,perProduct:S.cc,layout:S.am,total:total()};
  const products=S.s.querySelector('.products'),creatives=S.s.querySelector('.creatives'),arrangement=S.s.querySelector('.arrangement');
  const p=Math.min(20,Math.max(1,Number(products?.value)||1));
  const c=Math.min(10,Math.max(1,Number(creatives?.value)||1));
  const a=arrangement?.value==='together'?'together':'separate';
  S.pc=p;S.cc=c;S.am=a;
  return{products:p,perProduct:c,layout:a,total:a==='together'?c:p*c}
}
function quantitylock(){
  const p=S.pc,c=S.cc,n=total(),mode=S.am==='together'?'all products together':'one assigned product per output';
  return`EXACT CURRENT-RUN QUANTITY LOCK — HIGHEST PRIORITY AND FINAL AUTHORITY. Ignore every older product count, creative count, number or queue from previous messages, previous Virag runs, browser storage and conversation history. The current visible Virag settings are: ${p} distinct product${p===1?'':'s'}, ${c} creative${c===1?'':'s'} per product/layout selection, ${mode}, EXACT TOTAL = ${n} standalone output${n===1?'':'s'}. Generate exactly ${n} outputs and stop immediately after output ${n}. Fewer or more than ${n} is incorrect. Never reuse an older value such as 7 unless EXACT TOTAL above is currently 7.`
}
function cleanref(v){
...
function characterrealismrule(x){
  const h=S.mods.human||{},r=h.characterRealismSystem||{};
  const reject=Array.isArray(r.reject)?r.reject.join('; '):'AI-generated face; plastic or waxy skin; generic influencer look; identity drift; incorrect hands or anatomy; synthetic eyes; mannequin pose; pasted-in subject; mismatched lighting; CGI or illustration look when realism is required';
  const qa=Array.isArray(r.qa)?r.qa.join('; '):'inspect the full frame, face, skin, eyes, hair, hands, anatomy, pose, lighting and identity continuity';
  return[
    'REAL HUMAN CHARACTER MASTER — CONDITIONAL HARD LOCK.',
    'Apply whenever any human, model, influencer, presenter, actor, face, hand, body or human-like character appears in this '+x.tab+' output. Do not add a person when the current brief does not need one.',
    r.rule||'Every depicted person must look like a genuinely photographed human being in a physically plausible real environment—never AI-generated, illustrated, game-rendered, mannequin-like, waxy, airbrushed or beauty-filtered.',
    'IDENTITY: '+(r.identityLock||'When a current-turn person reference is supplied, preserve the same recognizable facial geometry, head shape, eye spacing, nose, lips, jaw, ears, skin tone, hairline, hairstyle, facial hair, age, body proportions and distinctive asymmetry across every output and frame. No beautification, slimming, symmetry correction, age change or identity drift.'),
    'SKIN: '+(r.skin||'Preserve pores, peach fuzz, fine lines, under-eye texture, subtle blemishes and natural tonal variation; reject plastic, porcelain, glowing, over-smoothed or oversharpened skin.'),
    'FACE AND BODY: '+(r.eyesHairTeeth||'Use credible eyes, gaze, catchlights, hair strands, flyaways, teeth and lips.')+' '+(r.anatomy||'Maintain correct hands, finger count, joints, limbs, body mechanics, garment tension, contact and occlusion.'),
    'PERFORMANCE: '+(r.expressionPose||'Use restrained micro-expressions, natural posture, believable weight and candid gesture—not frozen mannequin posing or theatrical acting.'),
    'CAMERA AND LIGHT: '+(r.camera||'Use physically plausible perspective, focal length, exposure, depth, sensor texture and lens behavior without facial distortion.')+' '+(r.lightingEnvironment||'Match subject and environment lighting, shadows, reflections, material response and contact.'),
    'MOTION: '+(r.motion||'For video, preserve identity and wardrobe across frames with human-speed motion, natural blinking, breathing and weight shifts; reject morphing, flicker, rubber limbs and sliding feet.'),
    'STYLE EXCEPTION: '+(r.explicitStyleException||'Only use a non-photoreal character when the current brief explicitly requests that exact style; never silently turn a requested real person into CGI or illustration.'),
    'REJECT: '+reject+'.',
    'FINAL CHARACTER QA: '+qa+'. Any failed face, hand, anatomy, lighting or identity check requires correction before delivery. This lock never weakens the current brief, Pixel-Locked Product Fidelity, real typography or separate-output rules.'
  ].join(' ')
}
...
function batchlock(x,row,pos,total,id){
  const image=['Creative','Flyer','3D','Packaging'].includes(x.tab);
  const exact=S.am==='together'?S.refs.slice(0,S.pc).join(' | '):(S.refs[row.product-1]||`Product-${row.product}`);
  const assigned=S.am==='together'?`all ${S.pc} current-turn product files together`:`Product ${row.product} only — current-turn attached original file: ${exact}`;
...
function promptrow(x,row,pos,rows,id,brief){
  ...
  return[
    ...
    A,
    B,
    brief?`ORIGINAL REQUEST\n${brief}`:'ORIGINAL REQUEST\nUse the current-turn attached product and current Virag settings.',
    rowLock,
...
function rememberfiles(list,replace=0){
  const incoming=[...(list||[])].filter(f=>f&&(/^image\//i.test(f.type||'')||/\.(?:png|jpe?g|webp|gif|avif|heic|heif|bmp|tiff?)$/i.test(f.name||'')));
  if(!incoming.length)return[];
  const out=replace?[]:[...(S.sourceFiles||[])],seen=new Set(out.map(filekey));
  for(const f of incoming){const k=filekey(f);if(!seen.has(k)){seen.add(k);out.push(f)}}
  S.sourceFiles=out;
  return out
}
